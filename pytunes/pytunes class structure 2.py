...
"playlist" (no quotes). ')
            if song_number == 'library':
                ...
                if library.dict.has_key(song_number):
                    if self._dict.has_key(song_number):
...
def import_songs():
    ...
    while line != '':
        ...
        library_dict[temp_list[6]] = [temp_list[0], temp_list[1], \
...
def display_playlist(library_list):
    ...
    for song in library_list:
        print song[0] + '\t' + song[5] + '\t' + song[1].ljust(song_width)+\
              song[2].ljust(artist_width) + song[3].ljust(album_width) + song[4]
